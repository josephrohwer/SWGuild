...
public class AddressBookController {
    ...
    public void addAddress() {
        ...
        System.out.print("Please Enter City, State, and Zip: ");
        String cityStateZip = console.readString("");

        Address ad = new Address(firstName, lastName, streetAddress, cityStateZip);
        try {
            book.addAddress(ad);
        } catch (Exception e) {
        }
    ...
    public void findAddress() {
        ...
        if (choice == 1) {
            ...
            List<Address> addressTemp = new ArrayList<>(book.findAddressState(state));
            if (addressTemp.size() > 0) {
                addressTemp.stream().forEach((a) -> {
                    System.out.println(a);
                });
            } else {
                System.out.println("No Addresses With That State Name Found.");
            }
        } else if (choice == 4) {
